...
public class OutletEndpoint {
    ...
    @ApiMethod(
            name = "list_by_time",
            path = "outlet/list_by_time",
            httpMethod = ApiMethod.HttpMethod.GET)
    public CollectionResponse<Outlet> list_by_time(@Nullable @Named("cursor") String cursor, @Nullable @Named("limit") Integer limit, @Named("timestamp") Long timestamp) {
        limit = limit == null ? DEFAULT_LIST_LIMIT : limit;

        Query<Outlet> query = ofy().load().type(Outlet.class).filter("outlet_updated_time >= ", timestamp).limit(limit);
