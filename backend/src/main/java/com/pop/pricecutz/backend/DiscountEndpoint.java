...
public class DiscountEndpoint {
    ...
    @ApiMethod(
            name = "list_by_time",
            path = "discount/list_by_time",
            httpMethod = ApiMethod.HttpMethod.GET)
    public CollectionResponse<Discount> list_by_time(@Nullable @Named("cursor") String cursor, @Nullable @Named("limit") Integer limit, @Named("timestamp") Long timestamp) {
        limit = limit == null ? DEFAULT_LIST_LIMIT : limit;

        Query<Discount> query = ofy().load().type(Discount.class).filter("disc_updated_time >= ", timestamp).limit(limit);
